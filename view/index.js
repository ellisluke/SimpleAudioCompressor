...
*/
class TestCompressor_View extends HTMLElement
{
    constructor (patchConnection)
    {
        ...
        this.audioWindow = [];
        this.audioWindowCounter = 0;

        this.patchConnection.addEndpointListener("out", value => {
            this.audioWindowCounter = this.audioWindowCounter % 4;
            this.audioWindow[this.audioWindowCounter] = value.data[1, 0];
            this.refreshDisplay(value.data[0][0]);
            this.audioWindowCounter++;
        }, 44000, true)
    }
    ...
    refreshDisplay(data)
    {
        this.querySelector ("#updateVal").innerText = `${data}`;
    }
    ...
    getHTML()
    {
        ...
        <div id="controls">
          ...
          <p id="updateVal">init</p>
        </div>`;
    }
}

window.customElements.define ("testcompressor-view", TestCompressor_View);
...
{
    return new TestCompressor_View (patchConnection);
}
